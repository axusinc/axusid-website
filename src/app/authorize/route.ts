...
function authorize500(
  errorDescription: string,
  meta: Record<string, unknown> = {},
): NextResponse {
  console.error("authorize_500", {
    error_description: errorDescription,
    ...meta,
  });
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error instanceof DatabaseConfigError) {
      return authorize500("OAuth database is not configured");
    }

    if (error instanceof Error && error.message.includes("SESSION_SECRET")) {
      return authorize500("OAuth session signing is not configured");
    }

    return authorize500("Authorization request failed", {
